...
public class HttpHeaderParser {
    ...
    public static Cache.Entry parseCacheHeaders(NetworkResponse response) {
        ...
        String headerValue = getHeader(headers, "Date");

        if (headerValue != null) {
            serverDate = parseDateAsEpoch(headerValue);
        }

        headerValue = getHeader(headers, "Cache-Control");
        if (headerValue != null) {
        ...
        headerValue = getHeader(headers, "Expires");
        if (headerValue != null) {
            serverExpires = parseDateAsEpoch(headerValue);
        }

        headerValue = getHeader(headers, "Last-Modified");
        if (headerValue != null) {
            lastModified = parseDateAsEpoch(headerValue);
        }

        serverEtag = getHeader(headers, "ETag");
    ...
    public static String parseCharset(Map<String, List<String>> headers, String defaultCharset) {
        List<String> contentType = headers.get(LiteHttpConfig.CONTENT_TYPE);
        if (contentType != null && contentType.size() > 0) {
            String[] params = contentType.get(0).split(";");
    ...
    public static String parseCharset(Map<String, List<String>> headers) {
        return parseCharset(headers, LiteHttpConfig.DEFAULT_CONTENT_CHARSET);
    }
